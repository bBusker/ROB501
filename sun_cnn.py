...
class CNN(torch.nn.Module):
    def __init__(self, num_bins): 
        ...
        nn_block = []
        nn_block += conv(3, 32, 3)
        nn_block += conv(32, 32, 3)
        nn_block += conv(32, 32, 3)
        nn_block += [torch.nn.MaxPool2d((3,3),stride=3)]
        nn_block += conv(32, 32, 3)
        nn_block += [torch.nn.Dropout(0.5)]
        nn_block += [torch.nn.Conv2d(32, 8, kernel_size=(2,5), padding=0)]
...
if __name__ == "__main__":
    ...
    dset_loaders = {x: torch.utils.data.DataLoader(dsets[x], batch_size=32, shuffle=True, num_workers=0) for x in ['train', 'val']}
